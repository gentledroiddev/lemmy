...
use lemmy_apub::{ApubLikeableType, ApubObjectType, EndpointType, generate_apub_endpoint};
...
impl Perform for CreateComment {
  ...
  async fn perform(
    ...
    let updated_comment: Comment = match blocking(context.pool(), move |conn| -> Result<Comment, LemmyError> {
      let apub_id =
